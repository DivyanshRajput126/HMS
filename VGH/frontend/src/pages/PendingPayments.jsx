...
export default function PendingPayments() {
  ...
  const load = async () => {
    const res = await fetchCustomerList({
      ...
      page_size: 10,
      q: search || undefined,
      pending_only: true, // Only fetch customers with pending payments
    });
    setRows(res.items);
    setPages(res.pages);
  };
  ...
  return (
    <div style={{ padding: "24px", maxWidth: "100%", boxSizing: "border-box", backgroundColor: "#f8f9fa", minHeight: "100vh" }}>
      <div style={{ marginBottom: "20px" }}>
        <h2 style={{ margin: 0, color: "#1a202c" }}>Pending Payments</h2>
        <p style={{ color: "#718096", marginTop: "4px" }}>Monitor customers with outstanding balances</p>
      </div>
